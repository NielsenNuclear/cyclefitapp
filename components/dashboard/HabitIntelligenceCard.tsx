  ...
  return (
    <div className="bg-white/5 border border-white/10 rounded-2xl p-5 space-y-4">
      <div className="flex items-center justify-between">
        <h3 className="text-sm font-semibold text-white">Habit Intelligence</h3>
        <span className="text-xs text-white/40">{analytics.sampleSize} sessions</span>
      </div>
      ...
      <div className="space-y-1.5">
        <div className="flex justify-between text-xs">
          ...
          <span className="text-white font-medium">{analytics.consistencyScore}/100</span>
        </div>
      ...
      <div className="grid grid-cols-3 gap-3">
        <div className="bg-white/5 rounded-xl p-3 text-center">
          <div className="text-lg font-semibold text-white">{streakLabel}</div>
          <div className="text-[10px] text-white/50 mt-0.5">Current streak</div>
        </div>
        <div className="bg-white/5 rounded-xl p-3 text-center">
          <div className="text-lg font-semibold text-white">
            {Math.round(analytics.completionRates.rate30d * 100)}%
          </div>
          <div className="text-[10px] text-white/50 mt-0.5">30-day rate</div>
        </div>
        <div className="bg-white/5 rounded-xl p-3 text-center">
          <div className="text-lg font-semibold text-white">
            {Math.round(analytics.resilience * 100)}%
          </div>
          <div className="text-[10px] text-white/50 mt-0.5">Resilience</div>
        </div>
      </div>
        ...
        <div className="grid grid-cols-2 gap-3">
          {patterns.bestWeekday && (
            <div className="bg-emerald-500/10 border border-emerald-500/20 rounded-xl p-3">
              <div className="text-[10px] text-emerald-400 mb-0.5">Best day</div>
              <div className="text-sm font-semibold text-white">{patterns.bestWeekday.label}</div>
          ...
          {patterns.worstWeekday && patterns.worstWeekday.completionRate < 0.65 && (
            <div className="bg-rose-500/10 border border-rose-500/20 rounded-xl p-3">
              <div className="text-[10px] text-rose-400 mb-0.5">Hardest day</div>
              <div className="text-sm font-semibold text-white">{patterns.worstWeekday.label}</div>
              <div className="text-[10px] text-white/40">
        ...
        <div className="space-y-2">
          {profile.coachingInsights.slice(0, 2).map((ins, i) => (
            <div key={i} className="bg-violet-500/10 border border-violet-500/20 rounded-xl p-3">
              <div className="text-xs font-semibold text-violet-300 mb-0.5">{ins.headline}</div>
              <p className="text-[11px] text-white/55 leading-relaxed">{ins.detail}</p>
            </div>
          ))}
        </div>
      )}
