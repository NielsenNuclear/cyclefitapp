...
const RISK_CONFIG: Record<LifestyleBurnoutLevel, { label: string; color: string; bar: string }> = {
  low:      { label: "Low",      color: "text-success", bar: "bg-emerald-500" },
  elevated: { label: "Elevated", color: "text-info",     bar: "bg-sky-500"     },
  high:     { label: "High",     color: "text-caution",   bar: "bg-amber-500"   },
  critical: { label: "Critical", color: "text-danger",    bar: "bg-rose-500"    },
};
  ...
  return (
    <div className="bg-white border border-border rounded-2xl p-5 space-y-4 shadow-[0_1px_12px_rgba(0,0,0,0.04)]">
      ...
      <div className="flex items-center justify-between">
        <h3 className="text-sm font-semibold text-ink">Lifestyle Burnout</h3>
      ...
      {report.signals.length > 0 && (
        <div className="space-y-1 pt-1 border-t border-border">
          <div className="text-[10px] text-ink-faint uppercase tracking-widest">Signals</div>
      ...
      {report.recommendations.length > 0 && (
        <div className="space-y-1 pt-1 border-t border-border">
          <div className="text-[10px] text-ink-faint uppercase tracking-widest">What to do</div>
          {report.recommendations.slice(0, 2).map((r, i) => (
            <div key={i} className="flex gap-2 items-start">
              <span className="text-success text-[9px] mt-0.5">→</span>
              <span className="text-[11px] text-ink-secondary">{r}</span>
            </div>
          ))}
        </div>
      )}
