...
const LEVEL_CONFIG: Record<MaturityLevel, { label: string; color: string; bar: string }> = {
  beginner:     { label: "Beginner",     color: "text-info",     bar: "bg-sky-500"     },
  developing:   { label: "Developing",   color: "text-success",    bar: "bg-teal-500"    },
  intermediate: { label: "Intermediate", color: "text-brand",  bar: "bg-violet-500"  },
  advanced:     { label: "Advanced",     color: "text-caution",   bar: "bg-amber-500"   },
};
...
export function TrainingMaturityCard({ maturity }: Props) {
  ...
  const trendColor =
    maturity.trend === "improving" ? "text-success"
    : maturity.trend === "stable"  ? "text-info"
    : "text-danger";
  ...
  return (
    <div className="bg-white border border-border rounded-2xl p-5 space-y-4 shadow-[0_1px_12px_rgba(0,0,0,0.04)]">
      ...
      <div className="flex items-center justify-between">
        <h3 className="text-sm font-semibold text-ink">Training Maturity</h3>
        <div className="flex items-center gap-2">
          <span className={`text-xs font-semibold ${trendColor}`}>{trendLabel}</span>
          <span className={`text-xs font-bold px-2 py-0.5 rounded-full bg-surface-hover ${cfg.color}`}>
            {cfg.label}
          </span>
        </div>
      </div>
      ...
      <div className="flex items-end gap-3">
        ...
        <div className="pb-1 text-xs text-ink-muted">
          Training age: <span className="text-ink-secondary">{trainingAge(maturity.daysTrainingAge)}</span>
        </div>
      </div>
      ...
      {/* Insight */}
      <p className="text-[11px] text-ink-muted leading-relaxed">{maturity.insight}</p>

      {/* Next level hint */}
      {maturity.nextLevel && maturity.weeksToNextLevel !== null && (
        <div className="pt-0.5 text-[11px] text-ink-muted text-center">
