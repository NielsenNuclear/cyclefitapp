...
**Superseded 2026-07-18 by Dashboard 2.0** (`docs/ux/UXStabilizationAudit.md` Batches 15-19), **and revised 2026-07-19 by Dashboard 3.0** (`docs/ux/UXStabilizationAudit.md` Batch 20, implementing Deliverable 7 — "Option E, Refined" — of the Dashboard 2.0 Executive Product Strategy report). Dashboard 2.0's five-layer model consolidated ~90 cards to ~35; Dashboard 3.0 finishes that consolidation (three sections it never touched — Lifestyle & Adherence, Performance Tracking, Insights & Analytics — still had the exact duplication pattern fixed everywhere else) and adds the one genuinely new structural move: a dedicated **Insights destination** that pulls exploratory/meta-intelligence content off the daily scroll entirely. Every screen still answers, in order: *How am I today? What should I do? Why? What should I know? How am I improving?* — Insights is where "how good is Axis, and what has it found" now lives, reached deliberately rather than scrolled past.

1. **Layer 1 — Today's Focus (always visible).** `WorkoutCard` now owns the full workout lifecycle in one component — idle hero, skip-reason capture, post-workout feedback, and rescue-mode/minimum-viable-workout banners are internal render branches (`mode: "idle" | "skip-reason" | "feedback" | "active" | "done"`), not five sibling cards. Confidence renders via `TrainingHeader`, inline in the card header, not a standalone `ConfidenceBadge` div. A compact, non-blocking check-in banner (`DailyCheckIn`) sits above it.
2. **"Body Today" umbrella (visual grouping, not a component).** A small uppercase label groups four Summary→Details accordions answering "what's true about my body right now": `RecoveryHubCard` (now also carries a "Cycle-phase guidance" section merged in from the old standalone `RecoveryCard`, which had been stranded in the unrelated Layer 4), Cycle Intelligence, `NutritionHubCard`, and `LifestyleCard`.
3. **"This Week" umbrella.** One `AccordionSection` housing `ThisWeekSection`, a tab-shell reusing `TrainingPlanHubCard`'s own nested-tab pattern one level up, replacing what were three separate accordions: **Plan** (`CoachingHubCard` + `TrainingPlanHubCard`), **Trends** (`MomentumCard` + `ProgressHubCard` + `ExerciseTrendsCard`), **Athlete** (`GoalsMilestonesHubCard` + `AthleteProfileHubCard`), **Consistency** (`AdherenceHubCard` — new, merges `AdherenceCard`/`AdherenceIntelligenceCard`/`HabitIntelligenceCard`; three genuinely distinct engines with overlapping display concepts, not literal duplicates, so each overlapping concept keeps its richest source and drops the others rather than a delete-one-keep-one merge). A "See your full Insights →" teaser card sits at its base.
4. **The Insights destination (`/insights`, not a dashboard layer).** Reached only via the This Week umbrella's teaser card — deliberately *not* a 6th bottom-nav item, since none of the reference apps considered during the strategy pass (Whoop/Oura/Apple Health/Garmin/Strava) give exploratory/meta content its own permanent nav slot. Houses the ten cards that used to make up the old Layer 3/4 "Performance Tracking," "Insights & Analytics," and "Axis Intelligence" sections, grouped by the question they answer rather than their old section name: **How am I trending** (`ForecastCard`, `PerformanceForecastCard`, `AdaptiveInsightsPanel`), **Explore my data** (`InsightDiscoveryCard`, `ExerciseAnalyticsCard`, `CorrelationExplorerCard`), **How good is Axis** (`ConfidenceAccuracyHubCard`, `WhatAxisLearnedHubCard`, `OutcomeIntelligenceHubCard`, `CapacityHubCard`). Reads from a cached snapshot (`lib/insights/insightsSnapshot.ts`) written by `/dashboard` rather than recomputing independently — the underlying state is otherwise 100% local to `DashboardPage`'s mount effect with no shared context, and an independent recompute on `/insights` risked double-firing side-effecting calls. A missing or stale (not-today) snapshot renders an explanatory empty state, never silently-wrong data.
5. **Layer 5 — Education & Exploration.** Unchanged. Low-emphasis, optional. `BodyStatusCard`, rendered at the very end of the page.

**The hub-card composition pattern** — **Summary (always visible) → single expand/collapse toggle → Details** (with `Divider()`/`SectionLabel()` internal sub-sections as needed) — is unchanged and remains the standard for any new dashboard card, including the two built for Dashboard 3.0 (`AdherenceHubCard`, and `RecoveryHubCard`'s new section). Every toggle button uses the shared `min-h-[44px] ... focus-ring` className pattern. Maturity gates (`if (!x) return null`) must render an explanatory locked/empty state instead, not a silent blank.

**Density preference (`lib/preferences/dashboardDensity.ts`, Settings only).** A three-level Focused/Standard/Full toggle controlling *only* which `AccordionSection`s default open — never which sections exist, their order, or their contents, and never asked at onboarding. It is deliberately narrower than the strategy report's original five-concept "choose your dashboard" question: no reference app lets users switch between fundamentally different information architectures, so Dashboard 3.0 ships one IA (this one) with a lightweight density knob instead. `AccordionSection`'s own per-section `localStorage` override always wins over the density default once a user has manually toggled a section — an explicit choice outranks a global default, by design.

Composition is still hand-placed JSX inside `AccordionSection`s, not a declarative registry — the same v1-is-acceptable caveat as before applies. A config-driven registry remains a reasonable later-phase investment if the dashboard grows again.
...
**Touch targets and focus rings (added 2026-07-18, Dashboard 2.0 Batch 19):** every interactive control needs a minimum 44×44px hit area — glyph-only buttons (a dismiss "×", an icon button) must pad or negative-margin their way to that size rather than relying on the visible glyph's own dimensions; `CoachingHubCard`'s dismiss buttons are the reference pattern (`min-w-[44px] min-h-[44px] -m-2.5 flex items-center justify-center`). Any custom tab/switcher control (not using the shared `Tabs` primitive) needs explicit `role="tablist"/"tab"/"tabpanel"` plus `aria-selected` and `id`/`aria-controls`/`aria-labelledby` wiring — `TrainingPlanHubCard`'s This Week/This Block/Next 4 Weeks switcher is the reference. The `.focus-ring` utility class (`app/globals.css`) is the house keyboard-focus treatment and should be applied to every custom interactive element that isn't already using `Button`/`Tabs`/`Accordion` (which already carry it) — Dashboard 2.0's own Layer 2-4 hub cards initially missed this on eleven toggle buttons despite Layer 1's `RecommendationWhyCard` using it correctly, an internal-consistency gap worth watching for in future batches, not just a first-time omission.

**Known gap, not yet closed:** `prefers-reduced-motion` is honored via Tailwind's `motion-safe:` variant in exactly one component (`WorkoutModeShell.tsx`) — the intended house pattern per §9's Motion System, but essentially unrolled everywhere else (94 files use plain, unconditional CSS transitions). This predates Dashboard 2.0 and is a dedicated future initiative, not a per-feature fix.
