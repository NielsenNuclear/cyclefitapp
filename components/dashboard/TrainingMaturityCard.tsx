...
const LEVEL_CONFIG: Record<MaturityLevel, { label: string; color: string; bar: string }> = {
  beginner:     { label: "Beginner",     color: "text-sky-400",     bar: "bg-sky-500"     },
  developing:   { label: "Developing",   color: "text-teal-400",    bar: "bg-teal-500"    },
  intermediate: { label: "Intermediate", color: "text-violet-400",  bar: "bg-violet-500"  },
  advanced:     { label: "Advanced",     color: "text-amber-400",   bar: "bg-amber-500"   },
};
...
export function TrainingMaturityCard({ maturity }: Props) {
  ...
  const trendColor =
    maturity.trend === "improving" ? "text-emerald-400"
    : maturity.trend === "stable"  ? "text-sky-400"
    : "text-rose-400";
  ...
  return (
    <div className="bg-white/5 border border-white/10 rounded-2xl p-5 space-y-4">
      ...
      <div className="flex items-center justify-between">
        <h3 className="text-sm font-semibold text-white">Training Maturity</h3>
        <div className="flex items-center gap-2">
          <span className={`text-xs font-semibold ${trendColor}`}>{trendLabel}</span>
          <span className={`text-xs font-bold px-2 py-0.5 rounded-full bg-white/8 ${cfg.color}`}>
            {cfg.label}
          </span>
        </div>
      </div>
      ...
      <div className="flex items-end gap-3">
        ...
        <div className="pb-1 text-xs text-white/35">
          Training age: <span className="text-white/55">{trainingAge(maturity.daysTrainingAge)}</span>
        </div>
      </div>
      ...
      {/* Insight */}
      <p className="text-[11px] text-white/45 leading-relaxed">{maturity.insight}</p>

      {/* Next level hint */}
      {maturity.nextLevel && maturity.weeksToNextLevel !== null && (
        <div className="pt-0.5 text-[11px] text-white/35 text-center">
