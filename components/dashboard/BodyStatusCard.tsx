...
function StatusChip({ state, count }: { state: MuscleState; count: number }) {
  ...
  return (
    <div className="flex items-center gap-1.5 bg-[#F5F3EE] rounded-full px-2.5 py-1">
      <div className="w-2 h-2 rounded-full flex-shrink-0" style={{ backgroundColor: color }} />
      <span className="text-[11px] font-medium text-[#1C1B18]">{count}</span>
...
function BodySilhouette({ snapshot }: { snapshot: BodyIntelligenceSnapshot }) {
  ...
  const dotColor = dominant ? STATE_COLOR[dominant] : "#EAE7DE";
  ...
  return (
    <div className="flex gap-3 items-center">
      ...
      >
        {/* Head */}
        <ellipse cx="22" cy="8" rx="6" ry="7" fill="#EAE7DE" />
        {/* Torso */}
        <rect x="12" y="17" width="20" height="28" rx="4" fill="#EAE7DE" />
        {/* Left arm */}
        <rect x="4" y="18" width="7" height="22" rx="3.5" fill="#EAE7DE" />
        {/* Right arm */}
        <rect x="33" y="18" width="7" height="22" rx="3.5" fill="#EAE7DE" />
        {/* Left leg */}
        <rect x="13" y="47" width="8" height="28" rx="4" fill="#EAE7DE" />
        {/* Right leg */}
        <rect x="23" y="47" width="8" height="28" rx="4" fill="#EAE7DE" />
        {/* Status dot overlay */}
      ...
      <div className="space-y-1.5">
        ...
        {snapshot.muscles.every(m => m.state === "unknown") && (
          <div className="text-[11px] text-[#9B9690]">Log workouts to see muscle status</div>
        )}
      </div>
    </div>
  );
...
export function BodyStatusCard({ snapshot }: BodyStatusCardProps) {
  ...
  return (
    <button
      ...
      className="w-full text-left bg-white rounded-2xl border border-[#EAE7DE] shadow-[0_1px_12px_rgba(0,0,0,0.04)] p-5 hover:bg-[#FAFAF8] active:bg-[#F5F3EE] transition-colors"
    ...
    >
      <div className="flex items-start justify-between mb-4">
        <div>
          ...
          <div className="text-[16px] font-semibold text-[#1C1B18]">Muscle Map</div>
          {injuredCount > 0 && (
            <div className="text-[11px] text-[#C0392B] mt-0.5 font-medium">
              {injuredCount} injury flag{injuredCount > 1 ? "s" : ""}
            </div>
          )}
        </div>
        <div className="text-[#C8C5BC] mt-0.5">
      ...
      {todayMuscles.length > 0 && (
        <div className="mt-4 pt-4 border-t border-[#F0EDE4]">
          <div className="text-[10px] font-semibold uppercase tracking-[0.10em] text-[#9B9690] mb-2">
            Today's Targets
          </div>
          <div className="flex flex-wrap gap-1.5">
            {todayMuscles
              ...
              .map(m => (
                <span
                  key={m.id}
                  className="text-[10px] font-medium px-2 py-0.5 rounded-full bg-[#EEEDFE] text-[#3C3489]"
                >
                  {m.displayName.replace(/ \((left|right)\)$/, "")}
                </span>
              ))}
            {todayMuscles.filter(m => !m.id.endsWith("_right")).length > 5 && (
              <span className="text-[10px] text-[#9B9690] px-1 py-0.5">
      ...
      <div className="mt-4 text-[11px] text-[#9B9690] flex items-center gap-1">
