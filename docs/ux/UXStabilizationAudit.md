...
### Batch 15 — Dashboard 2.0, Layer 1: Today's Focus ✅ IMPLEMENTED (2026-07-18)
**Scope, as executed:** the "AXIS DASHBOARD 2.0 BLUEPRINT" brief's Phase 0 usage audit (every dashboard surface classified Hero/Primary/Secondary/Contextual/Historical/Hidden/Developer-only) followed by a full card migration map (every one of ~90 dashboard cards accounted for as Kept unchanged / Merged / Promoted to Hero / Moved to Weekly Insights / Hidden until maturity / Moved to Workout Mode / Removed), approved by the user before any implementation began. Layer 1 makes the above-the-fold surface answer "how am I today / what should I do / why" without the eleven separate cards that previously competed for that space. Per the brief, the Adaptive Intelligence and Recommendation Engine were explicitly out of scope — this batch consumes their existing outputs as props, it does not change how any of them are computed.

**What shipped:** `RecommendationWhyCard.tsx` (new) merges four previously-separate "why" surfaces (`RecommendationExplanationCard`, `ExplainableRecommendation`, `RecommendationExplanation`, `ExplainabilityCard`) plus `TodayHighlights` into one Collapsed → Expanded → Detailed disclosure, using directional bars and qualitative labels (High/Moderate/Minor impact) in place of every raw contribution percentage the four source cards previously rendered independently. `DailyCheckIn.tsx` gained a compact, non-blocking banner mode (the user's explicit choice between banner-vs-blocking placement relative to the workout card) so the check-in prompt no longer competes with "start workout" for the first screen. `TrainingCard` (in `RecommendationCards.tsx`) collapsed its always-visible body behind a "More detail" toggle. `WorkoutHeroView.tsx` condensed three always-visible caption lines into one and moved `workoutRationale` behind the existing exercise-preview disclosure. `ConfidenceBadge`/`ConfidenceIndicator` adopted the brief's required vocabulary (Learning / Building Baseline / Moderate Confidence / High Confidence) in place of raw percentages and a bare "Low confidence" label. `LockedInsight.tsx` gained a permanent second line explaining *why* a feature is locked, not just a countdown — the brief's explicit "never show blank cards, explain why" requirement, made into a single shared component so every maturity gate across all four layers inherits it automatically.

**Files changed:** `components/dashboard/RecommendationWhyCard.tsx` (new), `components/dashboard/DailyCheckIn.tsx`, `components/dashboard/RecommendationCards.tsx`, `components/workout/WorkoutHeroView.tsx`, `components/dashboard/DeloadAlertCard.tsx`, `components/intelligence/ConfidenceBadge.tsx`, `components/intelligence/ConfidenceIndicator.tsx`, `components/ui/LockedInsight.tsx`, `app/dashboard/page.tsx`. **Removed** (superseded, folded into `RecommendationWhyCard`): `components/intelligence/RecommendationExplanationCard.tsx`, `components/intelligence/ExplainableRecommendation.tsx`, `components/dashboard/TodayHighlights.tsx`, `components/dashboard/ExplainabilityCard.tsx`, `components/dashboard/RecommendationExplanation.tsx`.

**Verification:** `tsc --noEmit` clean, 379/379 tests passing (unchanged — container/UI consolidation, no new pure logic). Live-verified at 1280×900 and 390×844: collapsed/expanded states, the compact check-in banner, and confirmed every deleted component had zero remaining JSX usages and zero remaining imports anywhere in the codebase before removal.

---

### Batch 16 — Dashboard 2.0, Layer 2: Daily Status ✅ IMPLEMENTED (2026-07-18)
**Scope, as executed:** consolidates Recovery, Cycle, and Nutrition intelligence — seventeen previously-separate cards — into three hub cards, one per domain, each answering a single question (Recovery: "Can I recover well today?"; Cycle: "How does my cycle affect training?"; Nutrition: "Am I fueling appropriately?"), each following the same Summary → single toggle → Details pattern established in Batch 15. `PhaseCard` and `SymptomSummaryCard` — today-only facts, not historical intelligence — moved out of the Cycle accordion into the Daily Status header instead, per the user's guardrail against merging unrelated domains into one mega-card.

**What shipped:** `RecoveryHubCard.tsx` merges `ReadinessCard`, `FatigueCard`, `RecoveryIntelligenceCard`, `FatigueInsightsCard`, `RecoveryOptimizationCard`, `RecoveryCapacityCard`, `RecoveryStatusCard`, and `PersonalRecoveryCard` — the client-side `computeCorrelations()`/`consecutiveSessionImpact()` calls `FatigueInsightsCard` used are preserved verbatim (a pre-existing SSOT deviation, out of scope to fix here — the brief bars recommendation-engine changes, not just formula changes). `CycleHubCard.tsx` merges `CycleIntelligenceCard`, `OvulationEstimateCard`, `TrainingWindowCard`, `RecoveryWindowCard`, `CyclePatternsCard`, `UpcomingTrendsCard`, and the cycle-forecast half of `PredictionCard` (`components/intelligence/PredictionCard.tsx`, also removed) — confirmed during the merge that `CycleIntelligenceCard`'s own inline training-window/ovulation/pattern sections were strictly thinner (same underlying objects, fewer fields) than the four dedicated standalone cards, so this card uses the richer versions' content throughout. `NutritionHubCard.tsx` merges `FuelingCard` and `NutritionIntelligenceCard`. `BurnoutPreventionCard` and `RecoveryCheckinCard` stayed as separate siblings inside the Recovery accordion — confirmed to be a different underlying model and a data-collection input respectively, not additional angles on the same "can I recover" question.

**A real bug found and fixed while reading source, not introduced by the merge:** `ForecastCard.tsx`'s probability ring used `fill="white"`/`rgba(255,255,255,...)` for its center-score text and track, apparently copy-pasted from a dark-canvas design that predates this card's white background — the score was invisible. Fixed by switching to `tokenColor.ink`/`tokenColor.inkMuted` fill and a `rgba(0,0,0,0.06)` track stroke; the same fix pattern recurred twice more in Batch 18 (`ReadinessConfidenceCard`, `CapacityCard`), suggesting the dark-canvas template was copied at least three times without the color swap.

**Files changed:** `components/dashboard/RecoveryHubCard.tsx` (new), `components/dashboard/CycleHubCard.tsx` (new), `components/dashboard/NutritionHubCard.tsx` (new), `components/dashboard/ForecastCard.tsx` (bug fix only, kept standalone), `app/dashboard/page.tsx`. **Removed** (17 files, folded into the three hub cards above): `ReadinessCard.tsx`, `FatigueCard.tsx`, `RecoveryIntelligenceCard.tsx`, `FatigueInsightsCard.tsx`, `RecoveryOptimizationCard.tsx`, `RecoveryCapacityCard.tsx`, `RecoveryStatusCard.tsx`, `PersonalRecoveryCard.tsx`, `CycleIntelligenceCard.tsx`, `OvulationEstimateCard.tsx`, `TrainingWindowCard.tsx`, `RecoveryWindowCard.tsx`, `CyclePatternsCard.tsx`, `UpcomingTrendsCard.tsx`, `FuelingCard.tsx`, `NutritionIntelligenceCard.tsx`, `components/intelligence/PredictionCard.tsx`.

**Verification:** `tsc --noEmit` clean, 379/379 tests passing. Live-verified at 1280×900 and 390×844, seeded onboarding + 2-day exercise history.

**Remaining technical debt (a mobile QA pass surfaced during Batch 47, fixed in a follow-up commit, not this one):** `CycleHubCard`'s top-level `if (!cycleAccuracy) return null` — inherited unchanged from the original `CycleIntelligenceCard`'s identical gate — left the single-card "Cycle Intelligence" accordion fully blank (not hidden, not explained) whenever fewer than two periods are logged. See "Mobile Polish" below for the fix; noted here because the bug's origin predates this batch.

---

### Batch 17 — Dashboard 2.0, Layer 3: Progress & Weekly Insights ✅ IMPLEMENTED (2026-07-18)
**Scope, as executed:** consolidates Progress, Athlete Development, and Training Plan intelligence — twenty-two previously-separate cards — into five hub cards, following the brief's explicit instruction that Layer 3 should read as encouraging placeholders for new users, never empty analytics.

**What shipped:** `ProgressHubCard.tsx` anchors `PerformanceHubCard` and merges in `ProgressCard`'s weekly-prescription/mesocycle-timeline/plateau-intervention/next-session-target sections plus `ProgressInsightsCard`'s client-side `computeProgressInsights()` (including `detectAllPlateaus()`, `epley1RM()`, `volumeProxy()` — copied verbatim, a pre-existing SSOT deviation preserved rather than fixed, same rationale as Batch 16). `ExerciseTrendsCard.tsx` merges `ExerciseIntelligenceCard`'s volume-trend-arrow content with `PerformanceTrendsCard`'s completion/RPE detail, one row per exercise instead of two separate lists answering overlapping questions. `GoalsMilestonesHubCard.tsx` merges `GoalProgressCard`, `GoalRoadmapCard`, `MilestoneCard`, and `AthleteDevelopmentCard`'s milestone section (four milestone-adjacent engines collapsed to one surface); its `useMemo`-wrapped client-side `computeGoalSnapshot()`/`computeGoalProgress()`/`computeGoalForecast()` calls are `GoalProgressCard`'s original pattern, preserved not redesigned. `AthleteProfileHubCard.tsx` merges `AthleteProfileCard`, `TrainingMaturityCard`, `TrainingSummaryCard`, and `AthleteDevelopmentCard`'s non-milestone content — `TrainingMaturityCard` and `AthleteDevelopmentCard`'s training-age section were confirmed to be the same question answered by two different engines (`lib/planning/trainingMaturity` vs. `lib/athlete/trainingAge`), neither a strict superset of the other, so both surface as two clearly-labeled rows rather than two competing cards. `CoachingHubCard.tsx` merges `CoachViewCard`, `AdaptiveInsightsCard`/`AdaptiveInsightsPanel` (a confirmed hard duplicate — identical `insights` prop — `Card` deleted, `Panel` kept), and `TrainingDecisionCard`, introducing `scaleLabel()`/`probabilityLabel()` qualitative-conversion helpers so volume/intensity/complexity scale factors and success-likelihood percentages render as words (Increased/Reduced/Standard, High/Moderate/Lower likelihood), never numbers. `TrainingPlanHubCard.tsx` merges `WeeklyPlanCard`, `TrainingBlockCard`, `PeriodizedCalendarCard`, and `PeriodizationCard` into one tabbed card (This Week / This Block / Next 4 Weeks) — three timescales of the same planning system, not three unrelated cards; `PeriodizationCard`'s block-progress section was a confirmed duplicate of `TrainingBlockCard`'s (same "Week X of Y" fact, two engines), resolved by keeping `PeriodizationCard`'s richer, phase-colored version.

**Files changed:** `components/dashboard/ProgressHubCard.tsx` (new), `components/dashboard/ExerciseTrendsCard.tsx` (new), `components/dashboard/GoalsMilestonesHubCard.tsx` (new), `components/dashboard/AthleteProfileHubCard.tsx` (new), `components/dashboard/CoachingHubCard.tsx` (new), `components/dashboard/TrainingPlanHubCard.tsx` (new), `app/dashboard/page.tsx`. **Removed** (22 files): `TrainingSummaryCard.tsx`, `ProgressionCard.tsx`, `PerformanceTrendsCard.tsx`, `WeeklyPlanCard.tsx`, `TrainingBlockCard.tsx`, `PeriodizedCalendarCard.tsx`, `CoachViewCard.tsx`, `ProgressInsightsCard.tsx`, `GoalProgressCard.tsx`, `AdaptiveInsightsCard.tsx`, `AdaptiveCoachCard.tsx`, `PeriodizationCard.tsx`, `ExerciseIntelligenceCard.tsx`, `TrainingDecisionCard.tsx`, `PerformanceHubCard.tsx`, `AthleteDevelopmentCard.tsx`, `GoalRoadmapCard.tsx`, `MilestoneCard.tsx`, `TrainingMaturityCard.tsx`, `components/athlete/AthleteProfileCard.tsx`, `components/athlete/LongitudinalTimelineCard.tsx`, `ProgressCard.tsx`.

**Verification:** `tsc --noEmit` clean (two TS narrowing bugs found and fixed during this batch — an optional-chain guard that didn't narrow across a `.map()` closure in `CoachingHubCard`, and a `.filter()` chained directly onto a literal that inferred too narrow a type in `TrainingPlanHubCard`), 379/379 tests passing. Live-verified at 1280×900 and 390×844.

**Remaining technical debt:** `AthleteProfileHubCard`'s top-level gate (`if (!profile && !trainingAge?.dataReady && !resilienceIndex?.dataReady && !hasSummary) return null`) let a low-confidence-but-truthy `profile` object bypass the null-return while none of the three summary blocks had anything to show, producing a blank body under the header — same bug class as Batch 16's `CycleHubCard` finding, same fix location (Mobile Polish, below).

---

### Batch 18 — Dashboard 2.0, Layer 4: Axis Intelligence ✅ IMPLEMENTED (2026-07-18)
**Scope, as executed:** consolidates the "how good is Axis" surfaces — confidence, calibration, accuracy, personalization, and outcome-optimization — twenty-one previously-separate cards into four hub cards, kept behind an "Advanced" divider so this layer never competes with or distracts from today's workout, per the brief's explicit Layer 4 requirement.

**What shipped:** `ConfidenceAccuracyHubCard.tsx` merges nine cards (`ReadinessConfidenceCard`, `PredictionAccuracyCard`, `CalibrationIntelligenceCard`, `AccuracyTimelineCard`, `ConfidenceDashboardCard`, `UserTrustCard`, plus `RecommendationEffectivenessCard` — caught mid-implementation as a legitimate omission from the original migration-map count and added transparently rather than silently dropped) around a shared `qualitative(pct)` helper (High/Moderate/Building/Early data bands with success/caution/info/muted coloring) and reusable `QualBar`/`QualRing` sub-components, so no raw percentage renders anywhere in this card except inside its own detail-only breakdown. `WhatAxisLearnedHubCard.tsx` merges `WhatAxisLearnedCard` (anchor), `PersonalResponseCard`, `SituationMemoryCard`, `PersonalizationCard`, and `CoachingMemoryCard` — the last one relocated from a misfiled position in the "Performance Tracking" accordion, where it had been living despite its own header literally reading "What Axis has learned." `OutcomeIntelligenceHubCard.tsx` merges `OutcomeIntelligenceCard` (anchor, bottleneck framing) and `OutcomeOptimizationCard` — a confirmed hard duplicate leverage-point box (identical `LeveragePoint` type) rendered once instead of twice. `CapacityHubCard.tsx` merges `CapacityCard` (anchor — ring, pillars, forecast) and `ExecutiveSummaryCard` — another confirmed hard duplicate (identical `CapacityScore`/`MomentumScore` types) — `ExecutiveSummaryCard`'s unique top-3 unified-insights list survives as its own section rather than being dropped. `SignalImportanceCard` and `PersonalizationCard`'s content is fully absorbed into `WhatAxisLearnedHubCard`; a stray duplicate standalone `<PersonalizationCard>` render left alongside it in `page.tsx` was caught and removed before this batch's verification pass, not after.

**Two real bugs found and fixed while reading source, not introduced by the merge:** the same dark-canvas ring-color bug from Batch 16 recurred in `ReadinessConfidenceCard` (folded into `ConfidenceAccuracyHubCard`) and `CapacityCard` (folded into `CapacityHubCard`) — third and final occurrence, same fix. Separately, an ESLint unused-var flag on `physiologyConf` in `WhatAxisLearnedHubCard` was resolved by rendering `physiologyConf.summary` rather than deleting the prop, since silently dropping content would have violated the user's "nothing valuable disappears" consolidation guardrail.

**Files changed:** `components/dashboard/ConfidenceAccuracyHubCard.tsx` (new), `components/dashboard/WhatAxisLearnedHubCard.tsx` (new), `components/dashboard/OutcomeIntelligenceHubCard.tsx` (new), `components/dashboard/CapacityHubCard.tsx` (new), `app/dashboard/page.tsx`. **Removed** (21 files): `PersonalizationCard.tsx`, `WhatAxisLearnedCard.tsx`, `PersonalResponseCard.tsx`, `SignalImportanceCard.tsx`, `ReadinessConfidenceCard.tsx`, `PredictionAccuracyCard.tsx`, `components/intelligence/CalibrationIntelligenceCard.tsx`, `components/intelligence/RecommendationEffectivenessCard.tsx`, `ConfidenceDashboardCard.tsx`, `AccuracyTimelineCard.tsx`, `UserTrustCard.tsx`, `OutcomeOptimizationCard.tsx`, `OutcomeIntelligenceCard.tsx`, `ExecutiveSummaryCard.tsx`, `CapacityCard.tsx`, `SituationMemoryCard.tsx`, `InsightsCard.tsx`, `CoachAccuracyCard.tsx`, `CoachingMemoryCard.tsx` (the latter two relocated out of "Performance Tracking", not lost).

**Verification:** `tsc --noEmit` clean (a `never`-type narrowing bug in `OutcomeIntelligenceHubCard` — a large `!!(A || B || C || D)` boolean expression narrowing `scorecard` to `never` at the point of property access — fixed by extracting each disjunct to its own named boolean before combining), 379/379 tests passing. Live-verified at 1280×900 and 390×844; a follow-up orphaned-import sweep (JSX removed but import line left dangling) found and removed 18 stray import lines across the touched files before this batch's commit.

**Remaining technical debt:** `WhatAxisLearnedHubCard`'s summary-visibility gate included a bare truthy check on `recoveryCapacity` (any object, even `confidence: "early"`) without ever rendering a Recovery Capacity line — inherited from the original `WhatAxisLearnedCard`'s identical bug, which this merge carried forward rather than introduced. Left an expanded card with a blank body instead of either real content or a "still collecting data" message. Fixed in Mobile Polish, below, alongside the two Batch 16/17 findings of the same class.

---

### Batch 19 — Dashboard 2.0, Navigation / Mobile Polish / Accessibility & Performance ✅ IMPLEMENTED (2026-07-18)
**Scope, as executed:** the brief's final three implementation phases — verifying the Dashboard → Start Workout → Workout Mode → Workout Complete → Dashboard round trip still reads as a deliberate transition rather than "just another accordion" (no code changes required, Workout Mode already handles this per Batch 14); a live mobile QA sweep at 390×844 across every Layer 2-4 hub card; and a dedicated accessibility audit of the new hub-card surface.

**What the mobile sweep found:** a horizontal-overflow scan across the fully-expanded dashboard (`document.body.scrollWidth` vs. `window.innerWidth`) found zero overflowing elements — the DS-2 token system and consistent card padding held up under every new hub card without a dedicated per-card mobile pass being necessary. It also found the three blank-body maturity-gate bugs flagged in Batches 16-18's "Remaining technical debt" notes above (`CycleHubCard`, `AthleteProfileHubCard`, `WhatAxisLearnedHubCard`) — in each case a top-level visibility gate used a loosely-truthy object check that didn't match what the render body actually needed to have content, all three inherited from the pre-Dashboard-2.0 source cards rather than introduced by the merge itself. Fixed by either rendering the content the gate was supposed to unlock (`WhatAxisLearnedHubCard`'s Recovery Capacity line, restored from the original `WhatAxisLearnedCard` and gated correctly this time) or adding the explanatory locked/empty state the brief's Data Maturity section requires instead of a silent blank shell (`CycleHubCard`, `AthleteProfileHubCard`).

**What the accessibility audit found:** `TrainingPlanHubCard`'s This Week/This Block/Next 4 Weeks switcher had no ARIA tab semantics (three plain buttons, no `role`, no selected-state exposure to a screen reader) and a 36px touch target below the app's 44px minimum — fixed with `role="tablist"/"tab"/"tabpanel"`, `aria-selected`, `id`/`aria-controls`/`aria-labelledby` wiring, and a 44px minimum height. `CoachingHubCard`'s three dismiss "×" buttons had only their ~14px glyph as a tap target — expanded to 44px via padding without changing the visible glyph size. Eleven hub cards' disclosure toggles used the shared `min-h-[44px]` pattern but were missing the `.focus-ring` utility that `RecommendationWhyCard` (Batch 15) already used — an inconsistency within Dashboard 2.0's own new surface, not a pre-existing gap, since all these toggle buttons were newly written in Batches 15-18. Fixed by adding `.focus-ring` to all eleven. Two gaps were identified and explicitly *not* fixed, as out of scope: `AdherenceCard`/`MinimumViableWorkoutCard` have the same small dismiss-button tap target issue but are pre-existing "Kept unchanged" cards per the approved migration map, not part of this initiative; and `prefers-reduced-motion` is honored via Tailwind's `motion-safe:` variant in exactly one pre-existing component (`WorkoutModeShell`, Batch 14) out of 94 files using plain CSS transitions — a codebase-wide gap that predates Dashboard 2.0 and is a separate, larger initiative.

**What the performance check found:** no code changes were needed. `AccordionSection`'s existing `hasOpened`-gated mounting (children never render into the DOM until their accordion is opened for the first time) already satisfies the brief's "deferred loading of historical analytics" requirement, since every Layer 2-4 hub card was correctly wired through that existing primitive rather than a new one. The three hub cards that do client-side engine computation (`ProgressHubCard`, `RecoveryHubCard`, `GoalsMilestonesHubCard`) already wrap it in `useMemo`, inherited from their source cards' existing pattern.

**Files changed:** `components/dashboard/CycleHubCard.tsx`, `components/dashboard/AthleteProfileHubCard.tsx`, `components/dashboard/WhatAxisLearnedHubCard.tsx`, `app/dashboard/page.tsx` (mobile-polish fixes); `components/dashboard/TrainingPlanHubCard.tsx`, `components/dashboard/CoachingHubCard.tsx`, and nine other hub cards (`.focus-ring` addition only) (accessibility fixes).

**Verification:** `tsc --noEmit` clean, 379/379 tests passing, ESLint clean on every touched file, both changes live-verified at 390×844 with zero console errors before commit.

**Remaining technical debt, explicitly deferred:** dynamic/code-split imports (`next/dynamic`) for Layer 3/4 are not implemented — the existing architecture eagerly bundles all dashboard components into one client bundle, which predates Dashboard 2.0 (if anything, consolidating ~90 cards into ~35 reduced total component surface rather than adding to it) and converting to code-splitting is a separate, higher-risk initiative better scoped on its own. A full color-contrast-ratio tool-measurement pass (as opposed to the hand-picked, screenshot-verified DS-2 token palette already in use) was not performed, consistent with the same gap already noted for Workout Mode in Batch 14.

---

### Batch 20 — Dashboard 3.0: Recommended Dashboard Implementation ✅ IMPLEMENTED (2026-07-19)
**Scope, as executed:** implements Deliverable 7 ("Recommended Dashboard — Option E, Refined") of the Dashboard 2.0 Executive Product Strategy report, plus the product owner's follow-up decision to ship a lightweight density preference instead of full dashboard-mode switching. The strategy report's own audit found that Dashboard 2.0's original 90→47 consolidation never touched three sections — Lifestyle & Adherence, Performance Tracking, Insights & Analytics (14 cards) — which still showed the exact duplication pattern (`AdherenceCard` next to `AdherenceIntelligenceCard`) fixed everywhere else. Seven sequential, independently-verified phases: Layer 1 consolidation, Body Today regroup + Recovery relocation, Consistency hub + This Week regroup, Insights destination scaffolding, moving the ten Insights-bound cards, density preference, and this documentation pass.

**Phase 1 — Layer 1 consolidation.** `WorkoutCard.tsx` absorbs `TrainingCard`, the standalone `ConfidenceBadge`, `SkipReasonCard`, `WorkoutFeedbackCard`, `MinimumViableWorkoutCard`, and `RescueModeCard` as new internal render branches (`mode: "idle" | "skip-reason" | "feedback" | "active" | "done"`) rather than six sibling components each with their own visibility gate. Fixed a pre-existing dead-code bug in `WorkoutHeroView.tsx` along the way: a confidence-label lookup keyed to `"high"|"moderate"|"low"|"minimal"` that never matched the real `ConfidenceLevel` type's `"High"|"Moderate"|"Building"|"Limited"|"Insufficient"` values, silently dead since the component was written.

**Phase 2 — Body Today regroup + Recovery relocation.** Added a "Body Today" umbrella label (visual only) above Recovery/Cycle/Nutrition. Relocated the standalone `RecoveryCard`'s phase-narrative content (`focus`/`body`/`sleepTarget`/`sleepNote`/`practices`/`stressNote`, plus `rationale` — defined on `RecoveryRecommendation` but never rendered anywhere before this) into `RecoveryHubCard` as a new "Cycle-phase guidance" Details section. Confirmed zero field-level overlap with `RecoveryHubCard`'s own 17 props before merging — this is a relocation-with-a-new-section, not a dedup, since the two cards answer entirely different questions (quantitative recovery analytics vs. qualitative cycle-phase rules). `RecommendationCards.tsx` deleted once `RecoveryCard` was its last live export.

**Phase 3 — Consistency hub + This Week regroup.** `AdherenceHubCard.tsx` merges `AdherenceCard` + `AdherenceIntelligenceCard` + `HabitIntelligenceCard` — unlike Dashboard 2.0's mostly-literal-duplicate merges, these three are genuinely distinct engines with overlapping *display concepts*: two different consistency-score computations, three different skip-risk verdicts, two different "best day to train" answers. Each overlapping concept keeps its richest source and drops the others (composite 4-pillar consistency over a single-number version; `AdherenceRiskForecast` — has protective factors — over the two plainer risk types; `BehaviorPatterns`' best-*and*-worst-day-with-completion-rate over a best-day-only string), while unique fields with no duplicate (sample size, 30-day rate, resilience) survive as bonus content rather than being dropped. `AdherenceCard`'s life-event picker stays in the Summary, not Details, since it's an input. `ThisWeekSection.tsx` provides the tab-switching chrome (reusing `TrainingPlanHubCard`'s own nested-tab pattern one level up) while `app/dashboard/page.tsx` keeps composing the actual cards as JSX slot props — matching the codebase's existing split between data-fetching and display rather than threading 20+ raw props through a new component. `ScheduleInsightsCard`/`UpcomingLifeEventsCard` moved to `/profile` (calendar inputs, not body readouts), recomputed independently from the same pure localStorage reads `/dashboard` already used — safe because both are read-only derivations with none of the double-side-effect risk (e.g. `logNutritionDay()`) that later ruled out the same approach for Insights.

**Phase 4 — Insights destination scaffolding.** `lib/insights/insightsSnapshot.ts` derives each of the ten Insights-bound cards' snapshot field from `ComponentProps<typeof Card>` rather than hand-duplicating each card's prop shape, so the cache type can't silently drift from what the card actually renders. The write effect in `page.tsx` is a *separate* `useEffect` depending on the underlying state directly, not appended to the tail of the existing mount effect — most of that effect's `setX(...)` calls pass inline expressions, so reading the *state* variables at the end of the same synchronous pass would have captured last render's stale values, not what was just computed. `app/insights/page.tsx` ships empty/stale/ready states; no `NAV_ITEMS` change.

**Phase 5 — Move the ten cards into Insights.** Grouped "How am I trending" (`ForecastCard`, `PerformanceForecastCard`, `AdaptiveInsightsPanel`) / "Explore my data" (`InsightDiscoveryCard`, `ExerciseAnalyticsCard`, `CorrelationExplorerCard`) / "How good is Axis" (the four former Layer 4 hub cards) — matching the strategy report's own tiering, not raw inheritance of the old section names. Removed their render calls from `/dashboard` along with the now-empty "Performance Tracking" and "Axis Intelligence" `AccordionSection`s. `EquipmentInsightsCard` — Performance Tracking's one non-Insights-bound survivor — moved to `/profile` next to equipment management (recomputed independently, same pure-read pattern as Phase 3's schedule-data move); its dedicated `useEquipmentData` hook was deleted once `/dashboard` was confirmed as its only consumer. A "See your full Insights →" teaser card (`BodyStatusCard`'s button-not-Link pattern) sits at the base of This Week as the sole entry point.

**Phase 6 — Density preference.** `lib/preferences/dashboardDensity.ts`: a three-level Focused/Standard/Full toggle, `defaultOpenFor(sectionId, density)` as a lookup table rather than scattered conditionals. This is deliberately narrower than the original brief, which asked whether users should choose between the strategy report's five dashboard concepts — declined after product-lead pushback (no reference app — Whoop/Oura/Apple Health/Garmin/Strava — lets users switch between fundamentally different information architectures) in favor of this lightweight, Settings-only knob that changes only default expand-state, never architecture. Settings gained a new "Dashboard" section with a 3-way pill extending the page's existing 2-button `Toggle` pattern rather than introducing `SegmentedControl` as a new cross-file dependency. Live-verified that `AccordionSection`'s own per-section `localStorage` override still wins over the density default once a section has been manually toggled — an explicit choice outranks a global default, by design, not a bug to route around.

**Files changed:** `components/dashboard/WorkoutCard.tsx`, `components/workout/WorkoutHeroView.tsx`, `hooks/useReadinessData.ts`, `components/dashboard/RecoveryHubCard.tsx`, `components/dashboard/AdherenceHubCard.tsx` (new), `components/dashboard/ThisWeekSection.tsx` (new), `lib/insights/insightsSnapshot.ts` (new), `app/insights/page.tsx` (new), `lib/preferences/dashboardDensity.ts` (new), `app/settings/page.tsx`, `app/profile/page.tsx`, `app/dashboard/page.tsx` (touched in every phase). **Removed:** `components/dashboard/SkipReasonCard.tsx`, `WorkoutFeedbackCard.tsx`, `MinimumViableWorkoutCard.tsx`, `RescueModeCard.tsx`, `RecommendationCards.tsx`, `AdherenceCard.tsx`, `AdherenceIntelligenceCard.tsx`, `HabitIntelligenceCard.tsx`, `hooks/useEquipmentData.ts`.

**Verification:** `tsc --noEmit` clean and 431/431 tests passing after every phase (437/437 from Phase 6 onward, +6 new unit tests for `defaultOpenFor`/`getDashboardDensity`/`setDashboardDensity`); ESLint cross-checked phase-by-phase against the pre-change baseline (identical rule-violation counts every phase — zero new debt introduced, confirmed via `git stash` A/B diffing, not just eyeballed). Live-verified at 1280×900 and 390×844 after every phase with zero console errors, including: full idle→active→done(partial)→feedback→idle and skip→skip-reason→idle cycles for Phase 1; empty/stale/ready states for `/insights` (including a manually-backdated snapshot to force the stale path); and all three density levels plus the manual-override-wins-over-density case for Phase 6.

**Remaining technical debt, explicitly deferred:** seven pre-existing documentation files still reference the now-deleted `RecommendationCards.tsx` by name (`docs/design/DS2Progress.md`, `docs/design/IconSystemAudit.md`, `docs/design/ComponentInventory.md`, `docs/dashboard-architecture.md`, `PROJECT_STATUS.md`, `components/dashboard/README.md`, and this file's own earlier batches) — left as historical record rather than retroactively edited, consistent with this document's own practice of not rewriting past batch entries. `MomentumCard` was not made a subject of the merges in Phases 3/5 despite `AdherenceHubCard`'s Summary already surfacing a momentum quick-chip via the same underlying `MomentumScore` — a plausible future dedup, not attempted here since the plan didn't call for it and removing display content without an explicit instruction would have violated the "nothing valuable disappears without justification" guardrail.
...
## Constraints Honored — Dashboard 2.0 (Batches 15-19)

The note directly above predates the Dashboard 2.0 initiative and correctly describes a state where no information-architecture changes had yet been proposed. Batches 15-19 are that initiative, executed against a separate brief ("AXIS DASHBOARD 2.0 BLUEPRINT") with its own explicit constraint, honored throughout: **the Adaptive Intelligence and Recommendation Engine were not redesigned.** Every hub card introduced across Batches 15-18 consumes existing engine outputs as props; none of the merges changed how a score, forecast, or recommendation is computed. Where a pre-existing client-side computation pattern was carried into a new hub card unchanged (`ProgressHubCard`'s `detectAllPlateaus()`, `GoalsMilestonesHubCard`'s `computeGoalSnapshot()`/`computeGoalProgress()`/`computeGoalForecast()`, `RecoveryHubCard`'s `computeCorrelations()`), it is called out by name in that batch's write-up as a preserved pre-existing SSOT deviation, not silently inherited or newly introduced. No feature, engine, or intelligence surface was deleted outright — every one of the ~90 cards inventoried in the Phase 0 usage audit is accounted for in the approved migration map with an explicit Kept/Merged/Promoted/Moved/Hidden/Removed status, and every "Removed" entry names the surviving destination that absorbed its content. Where a genuine pre-existing bug was found while reading source for a merge (the three ring-color rendering bugs, the three maturity-gate blank-body bugs), it is documented as found-not-introduced in the relevant batch, consistent with this document's established discipline of distinguishing a bug fix from a formula or architecture change.
