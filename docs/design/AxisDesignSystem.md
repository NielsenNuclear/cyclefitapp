...
**Superseded 2026-07-18 by Dashboard 2.0** (`docs/ux/UXStabilizationAudit.md` Batches 15-19). The three-layer model this section originally described (and the `DailyStatus`/`TodayHighlights` components it named) no longer exists — both were merged away during Dashboard 2.0's consolidation. The current model is five layers, and every screen is expected to answer, in order: *How am I today? What should I do? Why? What should I know? How am I improving?*

1. **Layer 1 — Today's Focus (always visible).** Greeting, hero workout card, readiness summary, the recommendation and its "why" (`RecommendationWhyCard`), Start Workout. A compact, non-blocking check-in banner (`DailyCheckIn`) sits above it. Nothing goes here unless a user should see it on every visit without expanding anything.
2. **Layer 2 — Daily Status.** One hub card per domain — `RecoveryHubCard`, `CycleHubCard`, `NutritionHubCard` — each answering exactly one question, each Summary-always-visible with a single toggle to Details. `PhaseCard`/`SymptomSummaryCard` (today-only facts) sit in the Daily Status header, not inside the Cycle accordion, since they aren't historical intelligence.
3. **Layer 3 — Progress.** `ProgressHubCard`, `ExerciseTrendsCard`, `GoalsMilestonesHubCard`, `AthleteProfileHubCard`, `CoachingHubCard`, `TrainingPlanHubCard` — weekly consistency, load trend, PRs, milestones. New users see encouraging locked-state messaging here, never empty analytics or a blank card.
4. **Layer 4 — Axis Intelligence.** `ConfidenceAccuracyHubCard`, `WhatAxisLearnedHubCard`, `OutcomeIntelligenceHubCard`, `CapacityHubCard`, kept behind a single "Advanced" divider so calibration/accuracy/personalization internals never compete with or distract from today's workout.
5. **Layer 5 — Education & Exploration.** Low-emphasis, optional. Currently just `BodyStatusCard`, rendered at the very end of the page.

**The hub-card composition pattern**, established across all fourteen Layer 2-4 cards and now the standard for any new dashboard card: **Summary (always visible) → single expand/collapse toggle → Details** (with `Divider()`/`SectionLabel()` internal sub-sections as needed). This satisfies "Hero → Summary → Details → Deep Analytics" without needing a literal fourth tier per card. Every toggle button uses the shared `min-h-[44px] ... focus-ring` className pattern — copy it rather than inventing a new disclosure treatment. Maturity gates (`if (!x) return null`) must render an explanatory locked/empty state instead, not a silent blank — see `CycleHubCard`'s and `AthleteProfileHubCard`'s locked-state blocks for the reference pattern, and `LockedInsight.tsx` for the shared component version when an `entryCount`-based countdown is available.

Composition is still hand-placed JSX inside `AccordionSection`s, not a declarative registry — the same v1-is-acceptable caveat as before applies, now more so given the card count roughly halved (from ~90 to ~35 meaningful surfaces) during the consolidation. A config-driven registry remains a reasonable later-phase investment if the dashboard grows again, but is not urgent immediately post-consolidation.
